...
- **Minimal global state**: The `Simulation` instance owns all runtime state (`window.sim` for console debugging). Design tokens (`window._PALETTE`, `window._FONT`, `window._r`) are frozen globals set by `colors.js` and consumed by ES modules via `window._PALETTE`. `_PALETTE.accentLight` is the hover-state accent color.
...
Pattern matches sibling `biosim/colors.js` — single source of truth with `_r(hex, alpha)` helper, frozen palette object, and CSS variable injection IIFE.
...
- Particle color is computed from `_PALETTE` charge hues (`chargePos=201` blue, `chargeNeg=7` red, `neutral='#847a70'` grey) with dynamic HSL saturation/lightness based on charge magnitude.
...
- Glass effect: `.glass` class (`background: var(--bg-panel)`, `backdrop-filter: blur(24px) saturate(1.3)`, `border: 1px solid var(--border)`, `box-shadow: var(--shadow-md)`) applied to topbar, panel-body, preset-content, hint-bar. Panel-body and preset-content override to `--shadow-lg`; hint-bar overrides to `--shadow-sm`.
...
### Design System
- Design system shared with sibling projects (`biosim`, `gerry` in same parent folder) — match their exact token values (colors, shadows, font sizes, spacing) when making UI changes. Accent: `#FE3B01`, accent light: `#FF6B3D`.
- CSS var naming convention: `--bg-canvas`, `--bg-panel`, `--bg-panel-solid`, `--bg-elevated`, `--bg-hover`, `--text`, `--text-secondary`, `--text-muted`, `--border`, `--border-strong`, `--accent`, `--accent-light`, `--accent-subtle`, `--accent-glow`. Layout tokens: `--toolbar-h`, `--panel-w`, `--radius-sm/md/lg/pill`, `--ease-out`, `--ease-in-out`, `--ease-spring`.
- Sidebar structure matches `biosim` — section headings (Geist 0.68rem/600/uppercase/0.12em tracking with `border-bottom` underline), margin-based section spacing (no inter-section borders), no description text under headings. Panel header: Geist 0.72rem same style.
