...
**Signal Delay** (`signalDelayEnabled`, requires Relativity):
- Finite-speed force propagation. Forces use each source particle's past position and velocity from its history buffer, solving the light-cone equation via Newton-Raphson iteration. Creates realistic lag in distant interactions.
