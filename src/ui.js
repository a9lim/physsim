...
export function setupUI(sim) {
    ...
    const infoData = {
        ...
        radiation: { title: 'Radiation', body: 'Accelerating charged particles radiate energy as photons (Larmor radiation: $P = 2q^2 a^2/3$). The full Landau\u2013Lifshitz reaction force $\\mathbf{F}_{\\!rad} = \\tau\\!\\left[\\dot{\\mathbf{F}}/\\gamma^3 - \\mathbf{v} F^2/(m\\gamma^2) + \\mathbf{F}(\\mathbf{v}\\!\\cdot\\!\\mathbf{F})/(m\\gamma^4)\\right]$ decelerates the emitter. The jerk term uses an analytical derivative for gravity and Coulomb, with a higher-order finite difference for other forces. Emitted photon particles carry the lost energy and momentum away.' },
