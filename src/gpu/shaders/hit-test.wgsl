...
fn main() {
    ...
    loop {
        ...
        if (isLeaf && pi >= 0) {
            ...
            if (nw != NONE && top < 47) { top += 1; stack[top] = u32(nw); }
            if (ne != NONE && top < 47) { top += 1; stack[top] = u32(ne); }
            if (sw != NONE && top < 47) { top += 1; stack[top] = u32(sw); }
            if (se != NONE && top < 47) { top += 1; stack[top] = u32(se); }
        }
    }
